...
def expand_table(df, factor, time_step):
    ...
    for j in df.columns.difference(["time"]):
        ndf.loc[:, j] = df[j].apply(make_list, factor=factor)
    ndf.loc[:, "time"] = df["time"].apply(make_interval, factor=factor, time_step=time_step)
    dc_col = {}
    for col in ndf.columns:
        dc_col[col] = ndf[col].explode()
...
class Mobility:
    ...
    def _fill_rows(self):
        ...
        self.rp.loc[:, self.repeats_float] = self.rp[self.repeats_float].astype(float).ffill()
        self.rp.loc[:, self.repeats_str] = self.rp[self.repeats_str].ffill()
        ...
        self.timeseries = _add_column_datetime(self.timeseries, self.totalrows, self.refdate, self.t)
        self.timeseries.loc[:, "count"] = self.timeseries.groupby(["hr", "state"])["distance"].transform("count")
        self.timeseries.loc[:, "distance"] = (self.timeseries.loc[:, "distance"] / self.timeseries.loc[:, "count"])
        self.timeseries = self.timeseries[["hh","state","weekday","category","distance"]]
    ...
    def run(self):
        ...
        if not self.profile.empty:
            if self.profile["state"].iloc[-1] == "driving":
                ...
                self.profile = self.profile.iloc[:-1 , :]
            ...
            self.profile.loc[self.hours - self.t, "departure"] = self.profile["departure"].iloc[-2]
            self.profile.loc[self.hours - self.t, "arrival"] = self.profile["arrival"].iloc[-2]
            self.profile.loc[self.hours - self.t, "last_arrival"] = self.profile["last_arrival"].iloc[-2]
            self.profile.loc[self.hours - self.t, "purpose"] = self.profile["purpose"].iloc[-2]
            self.profile.loc[self.hours - self.t, "duration"] = self.profile["duration"].iloc[-2]
            self.profile.loc[self.hours - self.t, "weekday"] = self.profile["weekday"].iloc[-2]
            self.profile.loc[self.hours - self.t, "category"] = self.user_defined
