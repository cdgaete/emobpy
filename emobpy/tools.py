...
def _add_column_datetime(df, totalrows, reference_date, t):
    ...
    drange = pd.date_range(start_date, periods=int(totalrows), freq=freq)
...
def localize_timeseries(df: pd.DataFrame, tz_local, dt=0.25):
    """
    The created emobpy time series is time-zone naive, but as the departure destination probabilities are in local time, the resulting time series must also correspond to local time
    However, emobpy does not seem to consider daylight saving time --> simply using tz_localize doesn't work

    :param df:          ga.timeseries
    :param tz_local:    time zone used for departure destination probabilities
    :param dt:          duration of one time step in minutes
    :return:
    """
    ...
    df.index = df.index.tz_localize(tz_local, ambiguous='NaT', nonexistent='NaT')

    # ensure that the state within the non-existing or ambiguous hour is 'home'
    if ((df.index.isna()) & (df.state != 'home') | ((df.state == 'home') & (df.soc != 1.0))).any():
        pass  # TODO implement adjustments to trip schedule and soc if needed

    # shift to summer time --> delete non-existing hour
    non_existent = (df.index.isna()) & (df['month'] == 3)
    df = df[~non_existent]

    # shift to winter time --> duplicate ambiguous hour
    for year in df['year'].unique():
        if df.index[df['year'] == year].isna().any():
            ambiguous_hour = df[(df.index.isna()) & (df['month'] == 10) & (df['year'] == year)]
            insert_pos = df.index.isna().nonzero()[0][0]  # position where to insert
            df = pd.concat([df.iloc[:insert_pos], ambiguous_hour, df.iloc[insert_pos:]])
            # directly resetting the index did not work --> add column datetime and reset the index
            df['datetime'] = df.index
            df.iloc[insert_pos:int(insert_pos + 2 / dt), df.columns.get_loc('datetime')] = pd.date_range(start=df.index[insert_pos - 1] + datetime.timedelta(minutes=dt * 60),
                                                                                                    end=df.index[int(insert_pos + 2 / dt)],
                                                                                                    inclusive='left',
                                                                                                    tz=tz_local,
                                                                                                    freq=f'{dt}H')  # use index before and after 'NaT' as start and end
            df.set_index('datetime', inplace=True)

    # drop added columns
    df = df[columns]

    # change column hh such that it increases with 15 minute resolution and does not show any jumps because of removed/added rows
    df.loc[:, 'hh'] = np.arange(0, len(df) * dt, step=dt)
