...
def build_model_balanced():
    ...
    model.battery_capacity = pyo.Param(model.ev, within=pyo.NonNegativeReals, default= 45.0) # kWh
    model.charge_efficiency = pyo.Param(model.ev, within=pyo.NonNegativeReals, default= 0.90) # percentage/fraction
    model.min_SOC = pyo.Param(model.ev, within=pyo.NonNegativeReals, default=0.0) # percentage/fraction
    model.max_SOC = pyo.Param(model.ev, within=pyo.NonNegativeReals, default=1.0) # percentage/fraction
    model.init_SOC = pyo.Param(model.ev, within=pyo.NonNegativeReals, default=0.5) # percentage/fraction
    model.grid_availability = pyo.Param(model.ev, model.T, within=pyo.NonNegativeReals, default=2) # default: 2 kW for each hour. See below, through input data can be modified.
    model.driving_consumption = pyo.Param(model.ev, model.T, within=pyo.NonNegativeReals, default=1) # kWh per time-step. Modify in input data by a time-series

    model.CHARGE_AT_BATTERY = pyo.Var(model.ev, model.T, domain=pyo.NonNegativeReals)
    model.SOC = pyo.Var(model.ev, model.T, domain=pyo.NonNegativeReals)
    model.CHARGE_ACC = pyo.Var(model.ev, model.T, domain=pyo.Reals)
    model.C_POS = pyo.Var(model.ev, model.T, domain=pyo.NonNegativeReals)
    model.C_NEG = pyo.Var(model.ev, model.T, domain=pyo.NonNegativeReals)
    model.INFEAS = pyo.Var(model.ev, model.T, domain=pyo.NonNegativeReals)

    def obj_expression(m):
        return pyo.quicksum(m.C_POS[ev,t] + 10000*m.INFEAS[ev,t] for t in m.T for ev in m.ev)
    ...
    def SOC(m, ev, t):
        if t == 1: # first timestep (Pyomo uses 1-based indexing)
            return m.SOC[ev, t] == m.init_SOC[ev] + m.CHARGE_AT_BATTERY[ev,t]*m.Δt/m.battery_capacity[ev] - m.driving_consumption[ev, t]/m.battery_capacity[ev]
        else:
            return m.SOC[ev, t] == m.SOC[ev, t-1] + m.CHARGE_AT_BATTERY[ev, t]*m.Δt/m.battery_capacity[ev] - m.driving_consumption[ev, t]/m.battery_capacity[ev]
    model.Constr_SOC = pyo.Constraint(model.ev, model.T, rule=SOC)

    def final_SOC(m, ev, t):
        if t == len(m.T):
            return m.SOC[ev, t] >= m.init_SOC[ev]
        else:
            return pyo.Constraint.Skip
    model.Constr_finalSOC = pyo.Constraint(model.ev, model.T, rule=final_SOC)

    def max_SOC(m, ev, t):
        return m.SOC[ev, t] <= m.max_SOC[ev]
    model.Constr_maxSOC = pyo.Constraint(model.ev, model.T, rule=max_SOC)

    def min_SOC(m, ev, t):
        return m.SOC[ev, t] >= m.min_SOC[ev]
    model.Constr_minSOC = pyo.Constraint(model.ev, model.T, rule=min_SOC)

    def charge_st_availability(m, ev, t):
        return m.CHARGE_AT_BATTERY[ev, t]/m.charge_efficiency[ev] <= m.grid_availability[ev, t] + m.INFEAS[ev, t]
    model.Constr_charge_st_avail = pyo.Constraint(model.ev, model.T, rule=charge_st_availability)

    def charge_st_battery(m, ev, t):
        return m.CHARGE_AT_BATTERY[ev, t]*m.Δt <= m.battery_capacity[ev]
    model.Constr_charge_st_batt = pyo.Constraint(model.ev, model.T, rule=charge_st_battery)

    def change_acceleration(m, ev, t):
        if t == 1:
            return m.CHARGE_ACC[ev, t] == 0
        else:
            return m.CHARGE_ACC[ev, t] == m.CHARGE_AT_BATTERY[ev, t] - m.CHARGE_AT_BATTERY[ev, t-1]
    model.Constr_charge_acc = pyo.Constraint(model.ev, model.T, rule=change_acceleration)

    def sign_charge(m, ev, t):
        return m.C_POS[ev, t] - m.C_NEG[ev, t] == m.CHARGE_ACC[ev, t]
    model.Constr_sign_charge = pyo.Constraint(model.ev, model.T, rule=sign_charge)
