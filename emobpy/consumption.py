...
class Weather:
    ...
    def _load_data(country_code="DE", year=2016, option="temp Kelvin", location=None):
        ...
        df = pd.read_csv(filename, parse_dates=["date"], date_format=dateparse, usecols=["date", country_code])
        timezones = country_timezones[country_code]
        df.date = pd.DatetimeIndex(df.date).tz_convert(timezones[0])
...
class Consumption:
    ...
    def _fill_rows(self):
        ...
        calcu = ["dayhrs"]
        mixed = repeats + fixed + variable_consumption + ["state"]
        ...
        rp.loc[:, repeats] = rp[repeats].astype(float).ffill()
        rp.loc[:, "state"] = rp["state"].ffill()
        rp.loc[:, fixed] = rp[fixed].astype(float).fillna(0)
        ...
        self.timeseries['consumption'] = (self.timeseries['consumption'] * self.timeseries['distance']).astype(float).fillna(0) / (
            self.timeseries['distance'].astype(float).replace(0.0, 1.0))  # this is an artifact to make zero the consumption when distance is zero
        self.timeseries.loc[:, "consumption"] = (self.timeseries.loc[:, "consumption"] / self.timeseries.loc[:, "count"])
